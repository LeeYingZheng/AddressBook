...
public class AddressBook {
    ...
    private static final String MESSAGE_UPDATED = "Updated person's contacts: %1$s, Phone: %2$s, Email: %3$s";
    ...
    private static final String UPDATE_TYPE_EMAIL = "email";
    private static final String UPDATE_TYPE_PHONE = "phone";
    private static final String UPDATE_TYPE_NAME = "name";
    ...
    private static final String COMMAND_UPDATE_DESC = "Updates a particular contact.";
    private static final String COMMAND_UPDATE_EXAMPLE = COMMAND_UPDATE_WORD + "John Doe phone p/96287593";
    private static final String COMMAND_UPDATE_PARAMETERS = "NAME CONTACT_VAR VAR_DETAILS";
    ...
    /**
     * Updates contact details for a particular person
     *
     * @param commandArgs
     * @return
     */
    private static String executeUpdatePerson(String commandArgs) {
        ...
        final ArrayList<String> splitArgs = splitByWhitespace(commandArgs);
        String updateType = getUpdateType(splitArgs);
        String updatedVar = getUpdateVar(splitArgs, updateType);
        if (isWrongFormat(splitArgs, updateType, INDEX_ZERO, splitArgs.size())){
            return getMessageForInvalidCommandInput(COMMAND_UPDATE_WORD, getUsageInfoForUpdateCommand());
        }
        int nameBoundary = splitArgs.indexOf(updateType);
        List<String> splitName = splitArgs.subList(0, nameBoundary);
        String actualName = "";
        //reassamble parts of name to fullname
        for (String namePart : splitName) {
            actualName += namePart + " ";
        }
        actualName = actualName.trim();

        String[] persontoUpdate = new String[PERSON_DATA_COUNT];
    ...
    private static String updatePersonContacts(String updateType, String updatedVar, String actualName, String[] persontoUpdate) {
        ...
        for (String[] person : getAllPersonsInAddressBook()) {
            if (getNameFromPerson(person).equals(actualName)) {

                switch (updateType) {
                    case UPDATE_TYPE_NAME:
                        updatePersonName(person, updatedVar);
                        break;
                    case UPDATE_TYPE_PHONE:
                        updatePersonPhone(person, updatedVar);
                        break;
                    case UPDATE_TYPE_EMAIL:
                        updatePersonEmail(person, updatedVar);
                        break;
                    default:
                }
                persontoUpdate = person;

                updatePersoninAddressBook();
    ...
    private static Boolean isWrongFormat(ArrayList<String> splitArgs, String updateType, int firstIndex, int secondIndex){
        return ((splitArgs.indexOf(updateType) == firstIndex) || (splitArgs.indexOf(updateType) == secondIndex));
    }

    private static void updatePersoninAddressBook() {
    savePersonsToFile(getAllPersonsInAddressBook(),storageFilePath);
    }

    private static void updatePersonName(String[] person, String newcontact){
        person[PERSON_DATA_INDEX_NAME] = newcontact;
    }

    private static void updatePersonPhone(String[] person, String newcontact){
        person[PERSON_DATA_INDEX_PHONE] = newcontact;
    }

    private static void updatePersonEmail(String[] person, String newcontact){
        person[PERSON_DATA_INDEX_EMAIL] = newcontact;
    }

    private static Boolean isUpdatePersonArgsValid(String commandArgs){
        final ArrayList<String> split = splitByWhitespace(commandArgs);
        if (split.size() < 3) { //must have at least one-word name, contact_var and var_details (3 total)
            return false;
        }
        else if ((!split.contains(UPDATE_TYPE_NAME)) && (!split.contains(UPDATE_TYPE_EMAIL)) && (!split.contains(UPDATE_TYPE_PHONE))) {
            return false;
        }
        else {
            return true;
    ...
    private static String getUpdateVar(ArrayList<String> updateInput, String updateType){
        if (updateType.equals(UPDATE_TYPE_NAME)){
            int startVar = updateInput.indexOf(updateType);
            List<String> splitName = updateInput.subList(startVar+1,updateInput.size());
            String updatedVar = "";
            for (String nameParts: splitName){
                updatedVar += nameParts + " ";
            }
            updatedVar = updatedVar.trim();
            return updatedVar;
        } else {
            return updateInput.get(updateInput.size() - 1);
        }
    }

    private static String getUpdateType(ArrayList<String> updateInput){
        if (updateInput.contains(UPDATE_TYPE_NAME)){
            return UPDATE_TYPE_NAME;
        }
        else if (updateInput.contains(UPDATE_TYPE_PHONE)){
            return UPDATE_TYPE_PHONE;
        }
        else {
            return UPDATE_TYPE_EMAIL;
        }
    }

    private static String getMessageForSuccessfulUpdatePerson(String[] updatedPerson) {
        return String.format(MESSAGE_UPDATED, getNameFromPerson(updatedPerson), getPhoneFromPerson(updatedPerson), getEmailFromPerson(updatedPerson));
    }
